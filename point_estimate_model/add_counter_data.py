...
NULL_PCT_MONTHLY_THRES = 25  # Threshold to drop null columns after aggregation.
...
def prepare_counter_data(data_file, cutoff_year):
  ...
  null_pct_monthly = df_monthly.isnull().mean() * 100 
  df_monthly = df_monthly.drop(null_pct_monthly[null_pct_monthly > NULL_PCT_MONTHLY_THRES].index, axis=1)
...
def main(provider, data_file, cutoff_year):
  ...
  # Save data
  df_smooth.to_pickle(f'./data/counter_data/people_counter_data_processed_{provider}.pkl')
