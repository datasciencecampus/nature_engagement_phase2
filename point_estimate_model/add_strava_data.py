...
def merge_strava_counter(provider, df):
    counter= pd.read_pickle(f'./data/counter_data/people_counter_data_processed_{provider}.pkl')
    ...
    # only merge 'total_trip_count' column
    merged= counter.merge(df[['total_trip_count', 'Date', 'site']], how='inner',left_on=['Date','site'],\
                                            right_on=['Date','site'])
