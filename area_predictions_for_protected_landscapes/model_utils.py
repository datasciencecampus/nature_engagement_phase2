...
def prepare_counter_data(data_file_cntr,cut_off_yr):
    ...
    rndm_site=random.randint(0, df_count.shape[1]-1)
    chsen_data=df_count.iloc[:,rndm_site]

    nam_site=chsen_data.name


    ax=df_raw.iloc[:,rndm_site].plot(style=['-o'],label='raw-data',legend=True)
    chsen_data.plot(ax=ax,style=['-*'],label='smooth-data',legend=True)
    str_up[rndm_site].plot(ax=ax,color='black',style=['--'],legend=False)
    str_low[rndm_site].plot(ax=ax,color='black',style=['--'],legend=False)
    plt.title(nam_site)
